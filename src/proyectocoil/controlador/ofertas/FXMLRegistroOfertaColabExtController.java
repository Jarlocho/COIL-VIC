 ...
 * Descripción: Muestra un formulario para registrar una nueva oferta externa
 * Versión: 1.0
 */
...
public class FXMLRegistroOfertaColabExtController implements Initializable {
    @FXML
    ...
    private OfertaColabExt obtenerInformacionOfertaColabExt() {
        OfertaColabExt ofertaExt = new OfertaColabExt();
        ofertaExt.setTema(tfTema.getText());
        ofertaExt.setFechaInicio(dpFechaInicio.getValue().toString());
        ofertaExt.setFechaTermino(dpFechaTermino.getValue().toString());
    ...
    private boolean irBusquedaProfExt() {
        OfertaColabExt ofertaExt = obtenerInformacionOfertaColabExt();
        boolean registroExitoso = false;
        try {
            Stage escenario = new Stage();
            FXMLLoader loader = Utils.obtenerLoader("vista/ofertas/FXMLBusquedaProfExt.fxml");
